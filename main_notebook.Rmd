...
Note that the `weightCol` parameter is necessary to distinguish the coverage easier.
...
Adding `facet = 'row'` will show each curve in a separate plot.
...
`vst()` was chosen over `rld()` because it is a much faster transformation.
...
Because this did not work, the chunks in this section will be not evaluated.
